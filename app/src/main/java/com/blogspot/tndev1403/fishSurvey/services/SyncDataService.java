package com.blogspot.tndev1403.fishSurvey.services;
...
import com.blogspot.tndev1403.fishSurvey.data.api.API;
import com.blogspot.tndev1403.fishSurvey.data.api.UpdateHost;
...
public class SyncDataService extends Service {
    ...
    FishCatchHandler catchedHandler;
    ArrayList<FishCatch> cathceds;
    Intent mIntent;
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        Init();
        catchedHandler = new FishCatchHandler(this);
    ...
    private void Init() {
        try {
            if (new UpdateHost().execute(Global.Host).get()) {
                Log.w(TAG, "Init: Success upadate host " + Global.Host);
                IS_FINISH_HOST = true;
            }
        } catch (Exception e) {
    ...
    public void onDestroy() {
        ...
        notificationManager.notify(Global.CODE.NOTIFICATION_REQUEST_CODE, notification);
        notificationManager.cancelAll();
        stopForeground(true);
        this.stopSelf();
    }

    void SyncRecord(final FishCatch catched) {
        Log.w(TAG, "SyncRecord: Chuyến đi từ ngày " + catched.getCreatedDate() + " đến ngày " + catched.getFinishTime());
        final JSONObject JSONSend = new JSONObject();
        try {
            JSONSend.put(API.Record.CAPTAIN_ID, Integer.parseInt(user.getUserID()));
            // --- TRIP --- //
            JSONObject trip = new JSONObject();
            trip.put(API.Record.trip.FROM_DATE, ProcessingLibrary.Using.DateTimeStringReverseFromTimeStamp(catched.getTripId()));
            trip.put(API.Record.trip.TO_DATE, catched.getFinishTime());
            ...
            JSONSend.put(API.Record.CATCHED_AT, catched.getCatchTime());
            Log.w(TAG, "Bat luc: " + catched.getCatchTime());
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    //region Send Infor
                    URL url = new URL(Global.getUpdateHostURL(Global.Record));
                    ...
                    int RecordID = -1;
                    String Respone = ProcessingLibrary.InputStreamToString(is);
                    ...
                    if (!catched.getImagePath().trim().isEmpty()) {
                        ...
                        for (String x : FileNames) {
                            String FileName = Global.CoppaFiles.APP_DIR + File.separator + x;
                            // --- Declare JSON
                            JSONObject ImageJSONObject = new JSONObject();
                            ImageJSONObject.put(API.Image.RECORD_ID, RecordID);
                            ImageJSONObject.put(API.Image.BASE64_CONTENT, ProcessingLibrary.Using.Base64FromImageFile(FileName));
                            // --- Send it
                            URL _url = new URL(Global.getUpdateHostURL(Global.Image));
                            ...
                            if (_conn.getResponseCode() == 200) {
                                ImageSent++;
                                if (ProcessingLibrary.Using.DeleteFile(FileName)) {
                                    catched.setImagePath(catched.getImagePath().replace(x, ""));
                                    catchedHandler.update(catched);
                                }
                            }
                            _conn.disconnect();
                        }
    ...
    void CleanFinishRecord(FishCatch catched) {
    ...
    void SyncRecords() {
        ...
        for (int i = 0; i < cathceds.size(); i++) {
            final FishCatch catched = cathceds.get(i);
            SyncRecord(catched);
        }
    ...
    void UpAndSyncCaptain() {
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    URL url = new URL(Global.getUpdateHostURL(Global.Captiain));
                    HttpURLConnection conn = (HttpURLConnection) url.openConnection();
                    conn.setRequestMethod("POST");
                    conn.setRequestProperty("Content-Type", "application/json"); //;charset=UTF-8
                    conn.setRequestProperty("Accept", "application/json");
...
//                    os.writeUTF(JSONSend.toString());
    ...
    private void GetAndCheckData() {
        String ID = preferences.getString(HomeScreenPresenter.ID_KEY, "");
        cathceds = catchedHandler.getNotSyncRecords(ID);
        if (cathceds.size() <= 0) {
            stopSelf();
            return;
        }
        for (int i = 0; i < cathceds.size(); i++) {
            if (cathceds.get(i).getFinishTime().trim().isEmpty()) {
                cathceds.remove(i);
            }
        }
    }
    ...
    void showNotification() {
        ...
        pendingIntent = PendingIntent.getActivity(this, Global.CODE.NOTIFICATION_REQUEST_CODE,
